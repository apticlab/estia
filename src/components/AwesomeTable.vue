<template>
	<div class="align-middle inline-block min-w-full">
		<div v-if="fields && !readonly" class="flex flex-row">
			<div :class="addResourceClass" class="ml-auto flex flex-row items-center cursor-pointer" @click="addRow()">
		...
		<table class="w-full table-auto border-collapse" :class="tableClass">
			...
			<tbody>
				<template v-for="(row, index) in rows">
					<tr :key="index" class="cursor-pointer" :class="rowClass" @click="selectable ? selectRow(row) : null">
						<td v-for="(header, index) in headers" :key="index" class="py-4 px-4">
							<div class="flex flex-row items-center h-full text-gray-dark" :class="getRowClass(header)">
								<div v-if="header.type == 'avatar'" class="flex flex-row items-center">
									<img
									...
									<div class="flex flex-col">
										<div class="mb-1 flex flex-row items-center">
											<a
												target="_blank"
												:href="ig_usr_url + '/' + deepPick(row, header.fields.title)"
												class="text-base mr-2 text-blue-500 hover:text-blue-600"
												>{{ deepPick(row, header.fields.title) }}</a
											>
											<span v-if="deepPick(row, 'usr.is_verified')" class="h-4 w-4">
												<svg-icon name="verified-badge" width="w-4" height="h-4" />
											</span>
											<span v-if="false" class="h-4 w-4">
												<svg-icon name="friendship-badge" width="w-4" height="h-4" class="bg-blue-400" />
											</span>
										</div>
								...
								<div v-if="header.type == 'link'" class="flex flex-row items-end">
									<a
										...
										target="_blank"
										:href="'https://www.instagram.com/p/' + deepPick(row, header.fields.url_link)"
								...
								<div v-if="header.type == 'username'" class="flex flex-row items-end">
									<a
										target="_blank"
								...
								<div v-if="header.type == 'hashtag'" class="flex flex-row items-end">
									<span class="font-medium underline text-blue-700 cursor-pointer"
								...
								<template v-if="header.type == 'stock'">
									<div class="flex flex-row">
										...
										<div
											class="ml-2"
											:class="deepPick(row, header.fields.flow) == 'plus' ? 'text--600' : 'text-red-500'"
										>
											<i
												class="mx-1"
								...
								<div v-if="header.type == 'pill'" class="flex flex-row items-center h-full">
									<span
										...
										:class="
											'pill-color-' + deepPick(row, header.field.color) ? deepPick(row, header.field.color) : 'green'
										"
								...
								<div v-if="header.type == 'friendship'" class="flex flex-row justify-center h-full w-full items-center">
									<span
										...
										:class="deepPick(row, header.field) ? 'bg-green-300' : 'bg-gray-200'"
									/>
								</div>
								...
								<div v-if="header.type == 'boolean'" class="flex flex-row justify-center h-full w-full items-center">
									<span
										class="rounded-full h-5 w-5"
										:class="deepPick(row, header.field) ? 'bg-green-300' : 'bg-red-300'"
									/>
								</div>
				...
				<tr v-if="!rows || rows.length == 0">
					<td :colspan="headers.length + 2">
						<div class="text-center text-gray font-bold text-md font-semibold bg-white py-5">
