...
export default {
  ...
  mounted: function () {
    this.$bus.on('modal-status-change', (data) => {
    ...
    this.$bus.on('modal-toggle', (config) => {
  ...
  methods: {
    ...
    saveNote: function () {
      ...
      this.$api.create('resource/messages', resourceMessage).then(
        (res) => {
          this.isLoading = false
          this.status = 'success'
        },
    ...
    uploadCSV: function (dismiss = false) {
      switch (this.fileUploaderState) {
        ...
        case 'import':
          ...
          this.$api.upload(formData, 'upload/' + this.data.resource).then(
            function (result) {
    ...
    closeModal (data) {
      this.isOpen = false
      this.$bus.emit('close-modal', data)
    },
    saveDate () {
      this.isLoading = true
