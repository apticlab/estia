    ...
    for (let i = 0; i < conditions.length; i++) {
      ...
      switch (condition.operator) {
        case "OR":
          ...
          conditionIsMet =
            conditionIsMet || evaluateCondition(_condition, object, reference);
  ...
  if (_.isFunction(conditionOperator)) {
    ...
    switch (conditionOperator) {
      ...
      case "AFTER":
        ...
        conditionIsMet = conditionFieldValue
          ? moment(conditionFieldValue).isAfter(conditionValue)
          : true;
        break;
      case "BEFORE":
        conditionValue = dateFromConditionValue(conditionValue);
        conditionIsMet = conditionFieldValue
          ? moment(conditionFieldValue).isBefore(conditionValue)
          : true;
        break;
      case "AFTER_OR_EQUAL":
        conditionValue = dateFromConditionValue(conditionValue);
        conditionIsMet = conditionFieldValue
          ? moment(conditionFieldValue).isSameOrAfter(conditionValue)
          : true;
        break;
      case "BEFORE_OR_EQUAL":
        conditionValue = dateFromConditionValue(conditionValue);
        conditionIsMet = conditionFieldValue
          ? moment(conditionFieldValue).isSameOrBefore(conditionValue)
          : true;
        break;
      case "LIKE":
