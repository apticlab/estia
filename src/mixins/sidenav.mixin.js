    ...
    collapseSideBar () {
      ...
      if (expanding) {
        ...
        if (this.is_mobile) {
          ...
          this.$bus.emit('side-bar:show-text', this.show_text)
        } else {
      ...
      this.$bus.emit('side-bar:collapse', this.is_collapsed)
    },
    listenForSideNavCollapseEvent () {
      this.$bus.on('side-bar:collapse', (value) => {
        this.is_collapsed = value
      })
      this.$bus.on('side-bar:show-text', (value) => {
